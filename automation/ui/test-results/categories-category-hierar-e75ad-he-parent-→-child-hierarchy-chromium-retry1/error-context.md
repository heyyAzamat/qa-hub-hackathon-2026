...
- Name: categories/category-hierarchy.spec.ts >> CAT-002b: breadcrumb reflects the parent → child hierarchy
- Location: tests/categories/category-hierarchy.spec.ts:90:5

# Error details

```
Error: Breadcrumb should contain text "Hierarchy_Parent_1776170509106"

expect(locator).toContainText(expected) failed

Locator: locator('[aria-label="breadcrumb"], nav[aria-label="breadcrumb"]').first()
Expected substring: "Hierarchy_Parent_1776170509106"
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Breadcrumb should contain text "Hierarchy_Parent_1776170509106" with timeout 5000ms
  - waiting for locator('[aria-label="breadcrumb"], nav[aria-label="breadcrumb"]').first()
