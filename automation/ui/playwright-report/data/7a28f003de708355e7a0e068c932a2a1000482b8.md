...
- Name: categories/category-crud.spec.ts >> CAT-002: edit a category name
- Location: tests/categories/category-crud.spec.ts:70:5

# Error details

```
Error: Category page heading should be "CAT002_Original_1776170347985"

expect(locator).toBeVisible() failed

Locator: getByRole('heading', { name: 'CAT002_Original_1776170347985' }).first()
Expected: visible
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Category page heading should be "CAT002_Original_1776170347985" with timeout 10000ms
  - waiting for getByRole('heading', { name: 'CAT002_Original_1776170347985' }).first()
...
> 134 |     ).toBeVisible({ timeout: 10_000 });
  ...
  149 |       .getByRole('alert')
  150 |       .getByText(/cannot be deleted|contains parts|not empty/i)
  151 |       .first();
  152 | 
  153 |     const eitherVisible = (await warningInDialog.isVisible()) || (await toastWarning.isVisible());
  154 | 
  155 |     expect(
  156 |       eitherVisible,
  157 |       'A warning about non-empty category should be shown when deleting a category with parts',
  158 |     ).toBe(true);
  159 |   }
  160 | }
  161 | 
